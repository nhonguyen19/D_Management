...
class GetTypeOfDevice extends StatelessWidget {
  ...
  GetTypeOfDevice({Key? key, required this.id, required this.displayString,required this.displayColor});
  @override
      ...
      child: FutureBuilder<List<TypeOfDiviceObject>>(
          ...
          builder: (context, snapshot) {
            if (snapshot.hasData) {
              ...
              return Text(
                '${displayString}${findTOD!.Type_Of_Device_Name.toString()}',
                style: TextStyle(
                  fontSize: 14,
                   color: displayColor
                   ),
...
class GetSupplier extends StatelessWidget {
  ...
  GetSupplier({Key? key, required this.id,required this.displayColor});
  @override
  Widget build(BuildContext context) {
    return Container(
      ...
      child: FutureBuilder<List<FacultyObject>>(
          ...
          builder: (context, snapshot) {
            if (snapshot.hasData) {
              ...
              return Text(
                '- Khoa: ${findfaculty!.facultyName.toString()}',
...
class GetBrand extends StatelessWidget {
  ...
  GetBrand({Key? key, required this.id, required this.displayString, required this.displayColor});
  @override
  Widget build(BuildContext context) {
    return Container(
  ...
  Widget build(BuildContext context) {
    return Container(
      child: FutureBuilder<List<ConfigurationDetailsObject>>(
          future: ConfigurationDetailsProvide.fetchConfigurationDetails(
              http.Client()),
          builder: (context, snapshot) {
            if (snapshot.hasData) {
              listConfigurationDetail = snapshot.data!;
              ConfigurationDetailsObject? findConfigurationDetails =
                  findGetConfigurationDetailsId(listConfigurationDetail!, id);
              return Text(
...
class GetConfigurationSpecification extends StatelessWidget {
  ...
  GetConfigurationSpecification(
      {Key? key, required this.id, required this.displayString});
  @override
  Widget build(BuildContext context) {
    return Container(
      child: FutureBuilder<List<ConfigurationSpecificationObject>>(
        future:
            ConfigurationSpecificationProvide.fetchConfigurationSpecification(
                http.Client()),
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            listConfigurationSpecificationObjects = snapshot.data!;
            List<ConfigurationSpecificationObject?>
                findConfigurationSpecification =
                findGetConfigurationSpecificationId(
                    listConfigurationSpecificationObjects!, id);
            return Column(      
                      children: [
                        for (int i = 0;
